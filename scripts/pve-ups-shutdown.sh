#!/bin/bash
# PVE UPS Manager - NUT shutdown hook
# Place in /etc/nut/upssched.conf or link from SHUTDOWNCMD in upsmon.conf
...
for vmid in $(qm list | awk '''{if(NR>1 && $2=="running") print $1}'''); do
...
for vmid in $(pct list | awk '''{if(NR>1 && $2=="running") print $1}'''); do
...
for i in $(seq 1 30); do
    running=$(qm list | awk '''{if(NR>1 && $2=="running") print $1}''' | wc -l)
    running_ct=$(pct list | awk '''{if(NR>1 && $2=="running") print $1}''' | wc -l)
    total=$((running + running_ct))
    if [ "$total" -eq 0 ]; then
        echo "All VMs/CTs stopped" >> $LOG
        break
    fi
    echo "Waiting... $total still running" >> $LOG
    sleep 5
done

# Shutdown host
echo "Shutting down host..." >> $LOG
shutdown -h +1 "UPS battery low - PVE UPS Manager initiated shutdown" >> $LOG 2>&1
